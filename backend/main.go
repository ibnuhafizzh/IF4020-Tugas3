...
func EncryptMessage(cipher interfaces.Cipher, ciphertext string) string {
  enc := cipher.Encrypt(ciphertext)
  return enc
}

func DecryptMessage(cipher interfaces.Cipher, plaintext string) string {
...
func encrypt(c *gin.Context) {
  ...
  if found {
    ...
    if idx != -1 {
      ...
      cipher = ciphers.NewCipher(message.Key)
      res = EncryptMessage(cipher, message.Body)
    }
  } else {
    cipher = ciphers.NewCipher(message.Key)
